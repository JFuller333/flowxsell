...
const Resume = () => {
  ...
  const atsResumeText = [
    ...
    "Phone: (323) 514-1461 | Email: flowxsell@gmail.com | LinkedIn: linkedin.com/in/jazlyn-fuller",
    "",
    "SUMMARY",
    "Shopify Plus developer focused on high-traffic storefront UX, B2B/wholesale flows, subscriptions, and conversion-focused PDP + cart experiences. Managed $1.3M+ in online sales and optimized large catalogs with scalable theme systems.",
    "",
    "CORE SKILLS",
    "Shopify Plus, Liquid, JavaScript, HTML, CSS, JSON, Custom sections, Theme architecture, B2B/Wholesale pricing + account gating, Subscriptions (ReCharge), Bundles, SEO, Performance optimization, GitHub",
    "",
    "TECHNICAL FRAMEWORKS",
    "Liquid, React, Next.js, Hydrogen, Remix, Storefront API, GraphQL, Node.js",
    "",
    "SELECTED PROJECTS",
    "TRU Supplements — gettrusupps.com",
    "- Built a custom bundle feature + bundle-ready layouts",
    "- Created reusable custom sections for rapid merchandising",
    "- Redesigned sister company website (high-traffic performance focus)",
    "- Launched Sculpt fitness challenge generating $100K in supplement sales & $50K in program revenue",
    "",
    "METPURE, INC. — metpure.com",
    "- Built wholesale discount rules + eligibility gating (B2B + conversion optimization)",
    "- Implemented B2B account registration and approval-ready segmentation",
    "- Managed and optimized 3,000+ product listings",
    "",
    "Let's Rebuild Tuskegee — donate.rebuildtuskegee.org",
    "- End-to-end headless Shopify storefront build (architecture → storefront → launch)",
    "- Storefront API + GraphQL integration powering performant UX",
    "",
    "InstaBalloons Wholesale — instaballoons.com",
    "- Bulk ordering workflows + customer-specific pricing",
    "- Helped redesign navigation menu UX for improved product discovery",
    "",
    "Curls Monthly — curlsmonthly.com",
    "- ReCharge subscription migration + customer portal updates",
    "- Custom subscription flow + product selection enhancements (Liquid + JavaScript)",
  ].join("\n");

  const handleDownloadAtsTxt = async () => {
    try {
      const blob = new Blob([atsResumeText], { type: "text/plain;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.download = "Jazlyn_Fuller_Shopify_Development_Resume_ATS.txt";
      link.href = url;
      link.click();
      URL.revokeObjectURL(url);
      toast.success("ATS resume downloaded (TXT)!");
    } catch (error) {
      console.error("Error downloading ATS TXT:", error);
  ...
  const handleDownloadAtsPdf = async () => {
    ...
    try {
      ...
      const addLines = (lines: string[], fontSize: number, fontStyle: "normal" | "bold" = "normal") => {
      ...
      // Header
      addParagraph("Jazlyn Fuller", 18, "bold");
      addParagraph("Shopify Developer & E-commerce Specialist", 12, "bold");
      addParagraph("Phone: (323) 514-1461 | Email: flowxsell@gmail.com | LinkedIn: linkedin.com/in/jazlyn-fuller", 10);
      y += 8;

      // Sections from the ATS text (simple parse by headings)
      const sections = [
        {
          title: "SUMMARY",
          body:
        ...
        },
        {
          ...
          body: "Liquid, React, Next.js, Hydrogen, Remix, Storefront API, GraphQL, Node.js",
        },
      ];

      for (const s of sections) {
        y += 6;
        addParagraph(s.title, 11, "bold");
        addParagraph(s.body, 10, "normal");
      }

      y += 10;
      addParagraph("SELECTED PROJECTS", 11, "bold");

      const projects: Array<{ title: string; bullets: string[] }> = [
        {
          title: "TRU Supplements — gettrusupps.com",
          bullets: [
            "Built a custom bundle feature + bundle-ready layouts",
            "Created reusable custom sections for rapid merchandising",
            "Redesigned sister company website (high-traffic performance focus)",
        ...
        {
          title: "Let's Rebuild Tuskegee — donate.rebuildtuskegee.org",
          bullets: [
            "End-to-end headless Shopify storefront build (architecture → storefront → launch)",
            "Storefront API + GraphQL integration powering performant UX",
          ],
        ...
        {
          title: "InstaBalloons Wholesale — instaballoons.com",
          bullets: [
            "Bulk ordering workflows + customer-specific pricing",
            "Helped redesign navigation menu UX for improved product discovery",
          ],
        ...
        {
          title: "Curls Monthly — curlsmonthly.com",
          bullets: [
            "ReCharge subscription migration + customer portal updates",
            "Custom subscription flow + product selection enhancements (Liquid + JavaScript)",
          ],
      ...
      for (const p of projects) {
        y += 8;
        addParagraph(p.title, 10, "bold");
        for (const b of p.bullets) {
          addParagraph(`- ${b}`, 10);
        }
      }

      pdf.save("Jazlyn_Fuller_Shopify_Development_Resume_ATS.pdf");
      toast.success("ATS resume downloaded (PDF)!");
    } catch (error) {
      console.error("Error downloading ATS PDF:", error);
      toast.error("ATS PDF download failed.");
    } finally {
          ...
          <div className="p-4 sm:p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-4">
              <div className="space-y-2">
                ...
                <h3 className="text-xl sm:text-2xl font-bold">ATS Resume (text-based)</h3>
                <p className="text-sm text-muted-foreground max-w-3xl">
                  Designed to parse cleanly in ATS/robot screening tools. Download as a text PDF (recommended) or TXT.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                ...
                >
                  <Download className="w-4 h-4 mr-2" />
                  {isDownloadingAtsResume ? "Preparing..." : "Download ATS PDF"}
                </Button>
                <Button
            ...
            <div className="rounded-lg border border-primary/10 bg-background/50 p-4 overflow-hidden">
              <pre className="text-xs leading-relaxed whitespace-pre-wrap text-foreground/90 font-mono">
                {atsResumeText}
              </pre>
            </div>
          </div>
